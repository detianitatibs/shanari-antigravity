...
| 分類                     | 技術                        | バージョン | 選定理由                                                                              |
| :----------------------- | :-------------------------- | :--------- | :------------------------------------------------------------------------------------ |
| **Web フレームワーク**   | Next.js                     | v16以上    | React ベースのモダンな開発、SSR 対応、開発体験の良さから選定。                    |
| **言語**                 | TypeScript                  | v5以上     | 静的型付けによる開発時のエラー検知とコード品質向上のため。                            |
...
| **テスト**               | Vitest, React Testing Library | 最新       | 標準的なテストツールであり、エコシステムが充実しているため。                          |
...
- **Storage:** ローカルの`data/`ディレクトリに配置された静的アセット、写真データなどを格納。
- **DB:** ローカルの`db/`ディレクトリに配置された SQLite3 ファイル。
